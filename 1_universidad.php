<?php include("menus/head.html")?>
<body>
    <div class="noise"></div>
    <div class="overlay"></div>
    <div class="terminal">

<div id="page-transitions">
<div class="page-preloader page-preloader-dark">
<div class="loader loader-frames"><i></i><i></i><i></i><i></i></div>
</div>
<div class="header header-light">
<a href="#" class="header-icon header-icon-1 open-left-sidebar"><i class="fa fa-navicon"></i></a>
<a href="index.php" class="header-logo scale-hover"></a>
<a href="#" class="header-icon header-icon-4 open-right-sidebar"><i class="fa fa-envelope-o"></i></a>
</div>
<div id="page-content">
<div id="page-content-scroll">



<br>
<br>
<br>
<br>
<div class="content-fullscreen" style="text-align:center;">
    <h1 class="tittle__shadow">UNIVERSIDADES</h1>
</div>
<div class="decoration decoration-margins"></div>
<div class="content-fullscreen">






    <div class="lista__uni">
    
            <div class="universidades" >
                <a href="#" class="button-green button button-center button-xs link__uni" id="mostrarUni1">
                AGRARIA LA MOLINA 
                <img src="imagenes/Universidades/logo1.png" class="icono__uni icono__">
                </a>
            </div>
                <p class="relleno__convenios" id="text1" >
                   <B>BENEFICIOS: <BR> </B>
                - TARIFA PREFERENCIAL EN CURSOS DE CAPACITACIÓN Y EXTENSIÓN  <br><br>
                -  BECA PARA HIJOS DEL PERSONAL DEL COBIENE Y CIVIL DEL COBIENE FALLECIDO EN ACCIÓN DE ARMAS O ACTO DE SERVICIO EN LA LUCHA CONTRA EL TERRORISMO O NARCOTRÁFICO, PREVIA ADMISION, EVALUACION SOCIOECONOMICA Y DISPONIBILIDAD DE LA OFICINA COMPETENTE DE LA UNALM.  <br><br>
                - TARIFA PREFERENCIAL EN LA PRE-AGRARIA 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                <img src="images/iconos/download2.svg" class="icono__">
                Descargar Ficha de Solicitud
                </a>
                </p>
                
        
   
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni2">
            CIENTÍFICA DEL SUR 
            <img src="imagenes/Universidades/logo2.png" class="icono__uni icono__">
            </a>
        </div>
             <p class="relleno__convenios" id="text2">
             <B>BENEFICIOS: <BR> </B>
            - DESCUENTO DEL 40% PARA LAS CARRERAS DE MEDICINA HUMANA, ENFERMERÍA Y OBSTETRICIA Y 60% DE DESCUENTO PARA LAS CARRERAS DE VETERINARIA Y ZOOTECNIA, ESTOMATOLOGÍA, ING. ECONÓMICA, COMUNICACIÓN, ING. AMBIENTAL, ING. AGROFORESTAL, ARQUITECTURA, ARTES ESCÉNICAS, BIOLOGÍA MARINA, NUTRICIÓN Y DIETÉTICA, PSICOLOGÍA, TURISMO SOSTENIBLE Y HOTELERÍA, NEGOCIOS SOSTENIBLES ING. DE SISTEMAS EMPRESARIALES, DERECHO, ING. DE ALIMENTOS, FARMACIA Y BIOQUÍMICA, SOBRE LA ESCALA “E” (NIVEL 1), APLICADAS A LAS ARMADAS MENSUALES. <br> <br> 
            - DESCUENTO DEL 30% EN LAS CARRERAS PROFESIONALES PARA LAS PERSONAS QUE TRABAJAN Y EL 30% DE DESCUENTO EN LOS CURSOS DE FORMACIÓN CONTINUA.      
             DESCUENTO DEL 10% EN MAESTRÍAS Y ESPECIALIZACIONES Y EL 15% DE DESCUENTO EN EL CENTRO DE IDIOMAS (INGLES – PORTUGUÉS) <br> <br> 
            - DOS (02) MEDIAS BECAS ANUALES EN PRE GRADO.
            <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                <img src="images/iconos/download2.svg" class="icono__">
                Descargar Ficha de Solicitud
            </a>
             </p>


        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni3">
            CATÓLICA DEL PERÚ 
            <img src="imagenes/Universidades/logo3.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text3">
            <B>BENEFICIOS: <BR> </B>
            - PRE GRADO : UBICA AL ALUMNO EN ESCALA DE PAGOS SEGÚN ESTUDIO REALIZADO EN LA UNIVERSIDAD. <br><br>
            - LOS HIJOS DE PERSONAL COBIENE FALLECIDO O DISCAPACITADO PODRÁN POSTULAR A SU PROGRAMA DE «BECAS POR SITUACIÓN ESPECIAL»
            <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                <img src="images/iconos/download2.svg" class="icono__">
                Descargar Ficha de Solicitud
            </a>
            </p>


        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni4">
            CESAR VALLEJO  
            <img src="imagenes/Universidades/logo4.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text4">
            <B>BENEFICIOS: <BR> </B>
                CATEGORIA ESPECIAL UCV – LIMA NORTE  S/.400.00 <br><br>
                CATEGORIA ESPECIAL UCV – LIMA ESTE   S/.350.00 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                <img src="images/iconos/download2.svg" class="icono__">
                Descargar Ficha de Solicitud
            </a>
            </p>


        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni5">
            ESAN 
            <img src="imagenes/Universidades/logo5.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text5">
                <B>BENEFICIOS: <BR> </B>
                10% DE DSCTO PARA ESTUDIOS EN SU CENTRO PRE UNV Y EN PRE GRADO 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>
        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni6">
            FEDERICO VILLARREAL 
            <img src="imagenes/Universidades/federico.jpg" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text6">
                <B>BENEFICIOS: <BR> </B>
                25% DE DSCTO  EN MAESTRÍAS Y DOCTORADOS (sólo titulares en actividad)
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni7">
            JAIME BAUSATE Y MESA 
            <img src="imagenes/Universidades/logo7.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text7">
                <B>BENEFICIOS: <BR> </B>
                20% DE DSCTO  EN EL PAGO DE MATRICULA Y PENSIÓN(PERIODISMO) SOLO PRE-   GRAD
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni8">
            MARCELINO CHAMPAGNAT 
            <img src="imagenes/Universidades/logo8.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text8">
                <B>BENEFICIOS: <BR> </B>
                10% DE DSCTO EN  PRE GRADO Y POST GRADO <br> <br> 30% DE DSCTO EN CENTRO PRE UNIV. 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>            
 
        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni9">
            NORBERT WIENER 
            <img src="imagenes/Universidades/logo9.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text9">
                <B>BENEFICIOS: <BR> </B>
                10% Y 15% DE DSCTO EN PENSIONES SEGÚN CARRERA DESDE EL INGRESO  
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>            


        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni10">
            DEL PACIFICO 
            <img src="imagenes/Universidades/logo10.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text10">
                <B>BENEFICIOS: <BR> </B>
                INCORPORAR EN EL SISTEMA DE ESCALA DE PENSIONES DE LA UNIVERSIDAD DESDE EL MOMENTO DE INGRESO, COMO ALUMNO DE PREGRADO LA UBICACIÓN EN LA ESCALA DE PAGOS, SE DETERMINA DE ACUERDO A LOS RESULTADOS DEL ANÁLISIS QUE REALICE LA GERENCIA DE PENSIONES <br><br> TARIFA PREFERENCIAL DEL 20 % DE DSCTO  EN EL CENTRO IDIOMAS. <br><br> SI LUEGO DE INGRESAR A LA UNIVERSIDAD (PREGRADO), EL ALUMNO PIERDE A SU PADRE O QUEDAN EN ESTADO DE DISCAPACIDAD, EL ALUMNO PUEDE SOLICITAR LA REVISIÓN DE LA ESCALA A LA GERENCIA DE PENSIONES.  
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni11">
            RICARDO PALMA 
            <img src="imagenes/Universidades/logo11.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text11">
                <B>BENEFICIOS: <BR> </B>
                CATEGORIZACIÓN A ESCALA MÍNIMA  EN PRE Y POST - GRADO <br><br> CRLS  DE 05 CUOTAS PAGAN  04 <br><br> DE TTE  CRL  A  SO3 Y EECC DE 05 CUOTAS PAGAN 03 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni12">
            SAN IGNACIO DE LOYOLA USIL 
            <img src="imagenes/Universidades/logo12.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text12">
                <B>BENEFICIOS: <BR> </B>
                OTORGARA LA CATEGORIA MINIMA (CATEGORIA “A”) DE PENSIONES EN EL PROGRAMA DE PRE GRADO <br><br> EL BENEFICIO SE CONSIDERARA UNA SOLA VEZ  Y SE CONSERVARA EN LA MEDIDA QUE EL ALUMNO MANTENGA UN BUEN RENDIMIENTO ACADEMICO, QUEDANDO ESTABLECIDO QUE EL ALUMNO QUE HAYA REPROBADO UNO O MAS CURSOS NO SE LE CONSEDERA NUEVAMENTE DICHO BENEFICIO, AUNQUE LOS SIGUIENTES PERIODOS TENGA NOTA APROBATORIA. 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni13">
            SAN JUAN BAUTISTA 
            <img src="imagenes/Universidades/logo13.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text13">
                <B>BENEFICIOS: <BR> </B>
                25% DE DSCTO EN LAS  PENSIONES, VARIA DE ACUERDO A LA CARRERA PROFESIONAL. EN PRE-POST GRADO  
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni14">
            SAN MARTÍN DE PORRES 
            <img src="imagenes/Universidades/logo14.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text14">
                <B>BENEFICIOS: <BR> </B>
                PRE GRADO: ESTABLECE ESCALAS DIFERENCIADAS PARA PERSONAL COBIENE Y CIVIL. LA UNIVERSIDAD INFORMA ANUALMENTE LAS VARIACIONES  EN LOS COSTOS DE CADA ESCALA.  <br><br> ALCANCE: PERSONAL COBIENE EN ACTIVIDAD O RETIRO E HIJOS, EMPLEADOS CIVILES EN ACTIVIDAD E HIJOS. <br><br>  POST GRADO: OTORGA 25% DE DESCUENTO PARA ESTUDIOS DE MAESTRÍAS, DOCTORADOS  Y SEGUNDA ESPECIALIZACIÓN. <br><br> SEDES :  SANTA ANITA, LA MOLINA, SURQUILLO Y JESÚS MARÍA.  <br><br> PRÓRROGA AUTOMÁTICA  
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>

        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni15">
            SAN MARCOS FACULTAD C.C. ECONOMICAS 
            <img src="imagenes/Universidades/logo15.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text15">
                <B>BENEFICIOS: <BR> </B>
                20% DEL COSTO TOTAL DEL PROGRAMA DE POSTGRADO QUE DESEEN CURSAR (DOCTORADO, MAESTRÍA Y DIPLOMADOS) A EXCEPCIÓN AL PROGRAMA DE DOBLE GRADO, CUYAS TARIFAS ESTÁN SUJETAS A LAS CONDICIONES QUE ESTABLECEN LAS UNIVERSIDADES EXTRANJERAS Y QUE NO DEPENDEN DE LA UNMSM
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>


        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni16">
            SEÑOR DE SIPAN 
            <img src="imagenes/Universidades/logo16.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text16">
                <B>BENEFICIOS: <BR> </B>
                20% EN MENSUALIDADES DE PENSIONES EN TODAS LAS CARRERAS (MODALIDAD A DISTANCIA) PRE- GRADO 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>


        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni17">
            UTP
            <img src="imagenes/Universidades/logo17.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text17">
                <B>BENEFICIOS: <BR> </B>
                25% DE DSCTO  EN EL PAGO DE PENSIONES EN EL PAGO EN PRE-POST GRAD
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>


        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni18">
            CATOLICA SEDE SAPIENTIAE 
            <img src="imagenes/Universidades/logo18.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text18">
                <B>BENEFICIOS: <BR> </B>
                ENTRE EL 25% AL 27% DE DESCUENTO DE ACUERDO A LAS CARRERAS) PARA ESTUDIOS DE PREGRADO. <br><br> 20% DEL COSTO TOTAL DEL PROGRAMA DE POSTGRADO (MAESTRÍAS Y DIPLOMADOS) A EXCEPCIÓN AL PROGRAMA DE DOBLE GRADO. 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>


        <!---->
        <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni19">
            UNIFE SAGRADO CORAZON 
            <img src="imagenes/Universidades/logo19.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text19">
                <B>BENEFICIOS: <BR> </B>
                PERSONAL OFICIALES (ACTIVIDAD Y RETIRO), Y FAMILIARES DIRECTOS (CÓNYUGE E HIJOS), SERÁN ASIGNADOS EN LA ESCALA Nº 28 (S/.890.00) Y PARA EL PERSONAL SUPERVISORES, TÉCNICOS Y SUBOFICIAL (ACTIVIDAD Y RETIRO) Y CIVILES EN ACTIVIDAD, EN AMBOS CASOS FAMILIARES DIRECTOS (CÓNYUGE E HIJOS), SERÁN ASIGNADOS EN LA ESCALA Nº 30 (S/.660.00), CABE RESALTAR QUE EL PAGO DE LAS PENSIONES DE ESTUDIO PARA EL PÚBLICO EN GENERAL ES DE (S/. 2500.00) <br><br> DESCUENTO DEL 20% AL CANCELAR EL CICLO COMPLETO DE ESTUDIOS EN EL CENTRO PRE UNIFÉ. 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>


       <!---->
       <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni20">
            UNIV. PRIVADA DEL NORTE 
            <img src="imagenes/Universidades/norte.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text20">
                <B>BENEFICIOS: <BR> </B>
                15% DE DESCUENTO EN LAS DIFERENTES CARRERAS PROFESIONALES DE PREGRADO. <br><br> 15% DE DESCUENTO, EN LAS DIFERENTES CARRERAS PROFESIONALES DE PREGRADO EN EL PROGRAMA WORKING ADULTO
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>


       <!---->
       <div class="universidades" >
            <a href="#" class="link__uni button-green button button-center button-xs" id="mostrarUni21">
            UNIV. PERUANA DE CIENCIAS APLICADAS UPC 
            <img src="imagenes/Universidades/logo21.png" class="icono__uni icono__">
            </a>
        </div>
            <p class="relleno__convenios" id="text21">
                <B>BENEFICIOS: <BR> </B>
                ESCALA MÍNIMA DE PENSIONES PARA LAS DIFERENTES CARRERAS PROFESIONALES DE PREGRADO, PROGRAMA DE ESTUDIOS PROFESIONALES PARA EJECUTIVOS (EPE) Y GENTE  QUE TRABAJA (EPE). (SOLO INGRESANTES A PARTIR DEL AF 2020) <br><br> OTORGAR LOS DESCUENTOS VIGENTES DETALLADOS EN LA POLÍTICA DE DESCUENTOS DE LA ESCUELA DE POSTGRADO - UPC EN CURSOS Y/O PROGRAMAS EDUCATIVOS SEGÚN CORRESPONDA. 
                <a href="https://drive.google.com/file/d/1E9wVieCur8Alr6B-XKSaF4tiTTFJQcHg/view?usp=sharing" class="link descargar__convenios" >
                    <img src="images/iconos/download2.svg" class="icono__">
                    Descargar Ficha de Solicitud
                </a>
            </p>
    </div>

    <script>
        $(document).ready(() => {
            $('#text1').hide();
            $('#text2').hide();
            $('#text3').hide();
            $('#text4').hide();
            $('#text5').hide();
            $('#text6').hide();
            $('#text7').hide();
            $('#text8').hide();
            $('#text9').hide();
            $('#text10').hide();
            $('#text11').hide();
            $('#text12').hide();
            $('#text13').hide();
            $('#text14').hide();
            $('#text15').hide();
            $('#text16').hide();
            $('#text17').hide();
            $('#text18').hide();
            $('#text19').hide();
            $('#text20').hide();
            $('#text21').hide();


            $('#mostrarUni1').click(function() {
                $('#text1').show();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });

            $('#mostrarUni2').click(function() {
                $('#text2').show();
                $('#text1').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });

            $('#mostrarUni3').click(function() {
                $('#text3').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });

            $('#mostrarUni4').click(function() {
                $('#text4').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni5').click(function() {
                $('#text5').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni6').click(function() {
                $('#text6').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni7').click(function() {
                $('#text7').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni8').click(function() {
                $('#text8').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni9').click(function() {
                $('#text9').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni10').click(function() {
                $('#text10').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni11').click(function() {
                $('#text11').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni12').click(function() {
                $('#text12').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni13').click(function() {
                $('#text13').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni14').click(function() {
                $('#text14').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni15').click(function() {
                $('#text15').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni16').click(function() {
                $('#text16').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni17').click(function() {
                $('#text17').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni18').click(function() {
                $('#text18').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text19').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni19').click(function() {
                $('#text19').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text20').hide();
                $('#text21').hide();
            });
            $('#mostrarUni20').click(function() {
                $('#text20').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text21').hide();
            });
            $('#mostrarUni21').click(function() {
                $('#text21').show();
                $('#text1').hide();
                $('#text2').hide();
                $('#text3').hide();
                $('#text4').hide();
                $('#text5').hide();
                $('#text6').hide();
                $('#text7').hide();
                $('#text8').hide();
                $('#text9').hide();
                $('#text10').hide();
                $('#text11').hide();
                $('#text12').hide();
                $('#text13').hide();
                $('#text14').hide();
                $('#text15').hide();
                $('#text16').hide();
                $('#text17').hide();
                $('#text18').hide();
                $('#text19').hide();
                $('#text20').hide();
            });
        });
    </script>

<div class="clear"></div>
</div>



<!--*********************************|BOTTON MENU|************************************+-->
<?php include("menus/menu-botton.html") ?>

<!--*********************************|LEFT MENU|************************************+-->
<?php include("menus/menu-left.html") ?>
<!--*******************************|FIN LEFT MENU|***********************************+-->

<!--*********************************|RIGHT MENU|************************************+-->
<?php include("menus/menu-right.html") ?>
<!--*******************************|FIN RIGHT MENU|***********************************+-->

<!--*********************************|RIGHT MENU|************************************+-->
<?php include("menus/menu-flotante.html") ?>
<!--*******************************|FIN RIGHT MENU|***********************************+-->




<div class="footer-menu-controls light-footer-menu">
<a href="#" class="footer-menu-open"><i class="fa fa-ellipsis-v bg-red-dark"></i></a>
<a href="#" class="footer-menu-close remove-menu"><i class="fa fa-times bg-red-dark"></i><em>Cerrar</em></a>
</div>
<div class="footer-menu-tap-close remove-menu"></div>
<div class="footer-menu-background light-footer-background remove-menu"></div>

<!--*********************************|RIGHT MENU|************************************+-->
<?php include("menus/menu-redes-sociales.html") ?>
<!--*******************************|FIN RIGHT MENU|***********************************+-->


<div class="sidebar-tap-close"></div>
<a href="#" class="back-to-top-badge"><i class="fa fa-caret-up"></i>Ir Arriba</a>
</div>
</body>